...
// src/integrations/platforms/web.ts
function createBrowserKaios(config) {
  const { persistToLocalStorage = true, ...rest } = config;
  return new Kaios({
    ...rest,
    stateBackend: persistToLocalStorage ? { type: "localStorage" } : { type: "memory" }
  });
}
function createUseKaios(_config) {
...
function createChatWidget(container, kaios) {
  injectStyles();
  ...
  const addMessage = (content, isUser, emotion) => {
...
exports.Kaios = Kaios;
exports.createBrowserKaios = createBrowserKaios;
exports.createChatWidget = createChatWidget;
exports.createUseKaios = createUseKaios;
exports.getEmotionClass = getEmotionClass;
