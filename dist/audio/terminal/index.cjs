...
var AudioBus = class extends events.EventEmitter {
...
var AudioRecorder = class {
  ...
  async checkSox() {
    return new Promise((resolve) => {
      const proc = child_process.spawn("sox", ["--version"]);
      proc.on("close", (code) => resolve(code === 0));
      proc.on("error", () => resolve(false));
    });
  }
  ...
  async getAudioDevices() {
    return new Promise((resolve) => {
      const proc = child_process.spawn("ffmpeg", ["-f", "avfoundation", "-list_devices", "true", "-i", '""'], {
  ...
  async reconstructAudio() {
    ...
    const tempDir = path.join(this.config.outputDir, ".temp_" + Date.now());
    try {
      fs.mkdirSync(tempDir, { recursive: true });
      const synthTempFiles = [];
      for (let i = 0; i < this.synthLog.length; i++) {
        const event = this.synthLog[i];
        const tempFile = path.join(tempDir, `synth_${i}.wav`);
        ...
        try {
          child_process.execSync(`sox ${soxArgs.join(" ")}`, { stdio: "ignore" });
          synthTempFiles.push({
            file: tempFile,
      ...
      const validSources = allSources.filter((s) => fs.existsSync(s.file));
...
var SamplePlayer = class {
   ...
   */
  getDefaultSoundsDir() {
    return path.join(process.cwd(), "assets/sounds");
  }
  /**
  ...
  async cacheSoundAvailability() {
    for (const mapping of SOUND_MAPPINGS) {
      const filePath = path.join(this.config.soundsDir, mapping.file);
      this.soundsCache.set(mapping.file, fs.existsSync(filePath));
    }
  }
  /**
   * Set enabled state
  ...
  async play(filename, _volume) {
    ...
    const filePath = path.join(this.config.soundsDir, filename);
    const exists = this.soundsCache.has(filename) ? this.soundsCache.get(filename) : fs.existsSync(filePath);
    if (!exists) {
      return;
    }
  ...
  hasSoundFile(filename) {
    ...
    const exists = fs.existsSync(path.join(this.config.soundsDir, filename));
...
var SoxSynth = class {
  ...
  async checkSox() {
    try {
      const proc = child_process.spawn("which", ["sox"]);
      proc.on("close", (code) => {
  ...
  async playNote(note, durationMs = 2500, velocity = 0.35) {
    ...
    try {
      const proc = child_process.spawn("play", args, {
...
exports.AMBIENT_SETS = AMBIENT_SETS;
exports.AmbientEngine = AmbientEngine;
exports.AudioBus = AudioBus;
exports.AudioRecorder = AudioRecorder;
exports.BASE_FREQUENCY = BASE_FREQUENCY;
exports.EMOTIONAL_SAMPLES = EMOTIONAL_SAMPLES;
exports.EMOTION_FREQUENCY_OFFSET = EMOTION_FREQUENCY_OFFSET;
exports.EMOTION_TO_MOOD = EMOTION_TO_MOOD;
exports.GLITCH_PATTERNS = GLITCH_PATTERNS;
exports.MUSIC_PROFILES = MUSIC_PROFILES;
exports.MusicEngine = MusicEngine;
exports.SCALE_432 = SCALE_432;
exports.SOUND_MAPPINGS = SOUND_MAPPINGS;
exports.SamplePlayer = SamplePlayer;
exports.SoxSynth = SoxSynth;
exports.TerminalAudio = TerminalAudio;
exports.ToneGenerator = ToneGenerator;
exports.createAmbientEngine = createAmbientEngine;
exports.createAudioBus = createAudioBus;
exports.createAudioRecorder = createAudioRecorder;
exports.createMusicEngine = createMusicEngine;
exports.createSamplePlayer = createSamplePlayer;
exports.createSoxSynth = createSoxSynth;
exports.createTerminalAudio = createTerminalAudio;
exports.createToneGenerator = createToneGenerator;
exports.extractSoundMarkers = extractSoundMarkers;
exports.getAudioBus = getAudioBus;
exports.getGlobalRecorder = getGlobalRecorder;
exports.getSoxSynth = getSoxSynth;
exports.setGlobalRecorder = setGlobalRecorder;
